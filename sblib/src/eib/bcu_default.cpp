...
class Timeout BusMeasurementtimeout;

BcuDefault::BcuDefault(UserRam *userRam, UserEeprom *userEeprom,
		ComObjects *comObjects, AddrTables *addrTables) :
		BcuBase(userRam, addrTables), userEeprom(userEeprom), memMapper(
				nullptr), usrCallback(nullptr), sendGrpTelEnabled(false), groupTelWaitMillis(
				DEFAULT_GROUP_TEL_WAIT_MILLIS), groupTelSent(millis()) {
...
void BcuDefault::_begin() {
	...
	// Vbus RUN level already checked in main(), now do remaining init of BCU and App and start normal operation
	BusVoltage::setup();
	// start the bus measurmenet timeout 10ms/40ms
	BusMeasurementtimeout.stop();
	BusMeasurementtimeout.start(bus_measurement_interval);
...
#   endif
	if (userRam != nullptr) {
		IF_DEBUG(
				serial.print("userRam    start: 0x", (unsigned int)userRam->startAddr(), HEX, 4); serial.print(" "););
		IF_DEBUG(
				serial.print("end: 0x", (unsigned int)userRam->endAddr(), HEX, 4); serial.print(" "););
		IF_DEBUG(
				serial.println("size: 0x", (unsigned int )userRam->size(), HEX,
						4));
	}
	if (userEeprom != nullptr) {
		IF_DEBUG(
				serial.print("userEeprom start: 0x", (unsigned int)userEeprom->startAddr(), HEX, 4); serial.print(" "););
		IF_DEBUG(
				serial.print("end: 0x", (unsigned int)userEeprom->endAddr(), HEX, 4); serial.print(" "););
		IF_DEBUG(
				serial.print("size: 0x", (unsigned int)userEeprom->size(), HEX, 4); serial.print(" "););
		IF_DEBUG(
				serial.println("flashSize: 0x",
						(unsigned int )userEeprom->flashSize(), HEX, 4)
				;);
	}
...
void BcuDefault::begin(int manufacturer, int deviceType, int version) {
	...
	userRam->status() = BCU_STATUS_LINK_LAYER | BCU_STATUS_TRANSPORT_LAYER
			| BCU_STATUS_APPLICATION_LAYER | BCU_STATUS_USER_MODE;
...
void BcuDefault::loop() {
	...
	// check if bus voltage OK, if failed inform app and start power down phase
	// we check every x (10/40)ms the state of the bus voltage level
	if (BusMeasurementtimeout.expired()) {
		//check Bus voltage is above 20V
		enum OPERATION_STATE state;
		state = BusVoltage::CheckBusVoltage();
		DB_BUSVOLTAGE(
				serial.println(" bcu loop check busvoltage state: ", state)
				;);
		if ((state != POWER_UP_OPERATION) && (state != POWER_DOWN_OPERATION)) {
			//bus failure inform APP
			//BusVoltage::appusersaveCallback->Notify( UsrCallbackType::storeAppBusVoltageFail);
			DB_BUSVOLTAGE(serial.println(" bcu loop check busvoltage: failed")
			; );
			if (ptr_appusersaveCallback != nullptr) {
				ptr_appusersaveCallback->Notify(
						UsrCallbackType::storeAppBusVoltageFail);

			} //set BCU end and reset controller
			DB_BUSVOLTAGE(
					serial.println ("!!!!!!!!!!!!!!!!!! bcu end and system reset !!!!!!!!!!!!" ); delay(1000); // give the serial line time to print buffer
					);
			BcuDefault::end();
			NVIC_SystemReset(); //should never return
		}

		BusMeasurementtimeout.start(bus_measurement_interval); // restart timeout
	...
	if (userEeprom->isModified() && !directConnection()
			&& userEeprom->writeDelayElapsed()) {
...
void BcuDefault::setAppUserSaveCallback(AppUserSaveCallback *callback) {
	ptr_appusersaveCallback = callback;
}
 ...
 */
bool BcuDefault::processGroupAddressTelegram(ApciCommand apciCmd,
		uint16_t groupAddress, unsigned char *telegram, uint8_t telLength) {
	DB_COM_OBJ(
			serial.println();
			serial.print("BCU grp addr: 0x", (unsigned int)groupAddress, HEX, 4);
	);

	comObjects->processGroupTelegram(groupAddress, apciCmd & APCI_GROUP_MASK,
			telegram);
	return (true);
}

bool BcuDefault::processBroadCastTelegram(ApciCommand apciCmd,
...
bool BcuDefault::processApciMemoryWritePDU(int addressStart, byte *payLoad,
		int lengthPayLoad) {
	DB_MEM_OPS(
	...
	return processApciMemoryOperation(addressStart, payLoad, lengthPayLoad,
			false);
}

bool BcuDefault::processApciMemoryReadPDU(int addressStart, byte *payLoad,
		int lengthPayLoad) {
	DB_MEM_OPS(
	...
	bool result = processApciMemoryOperation(addressStart, payLoad,
			lengthPayLoad, true);
...
bool BcuDefault::processApciMemoryOperation(unsigned int addressStart,
		byte *payLoad, unsigned int lengthPayLoad, const bool &readMem) {
	if (lengthPayLoad == 0) {
	...
	while ((!startNotFound) && (addressStart <= addressEnd)) {
		...
		if (memMapper != nullptr) {
			...
			if (startFound && endFound) {
				...
				if (readMem)
					operationResult = memMapper->readMemPtr(addressStart,
							&payLoad[0], lengthPayLoad) == MEM_MAPPER_SUCCESS;
				else
					operationResult = memMapper->writeMemPtr(addressStart,
							&payLoad[0], lengthPayLoad) == MEM_MAPPER_SUCCESS;
				...
				for (unsigned int i = 0; i < lengthPayLoad; i++) {
					...
					if (operationResult) {
						if (readMem)
							operationResult = memMapper->readMemPtr(
									addressStart, &payLoad[0],
									1) == MEM_MAPPER_SUCCESS;
						else
							operationResult = memMapper->writeMemPtr(
									addressStart, &payLoad[0],
									1) == MEM_MAPPER_SUCCESS;
					}
		...
		if ((userRam->inRange(addressStart, addressEnd))
				|| ((userRam->isStatusAddress(addressStart))
						&& (userRam->isStatusAddress(addressEnd)))) // USER_RAM_STATUS_ADDRESS (0x60) is system state specific
				{
			// start & end are in UserRAM
			if (readMem)
				userRam->cpyFromUserRam(addressStart, &payLoad[0],
						addressEnd - addressStart + 1);
			else
				userRam->cpyToUserRam(addressStart, &payLoad[0],
						addressEnd - addressStart + 1);
...
bool BcuDefault::processApci(ApciCommand apciCmd, unsigned char *telegram,
		uint8_t telLength, uint8_t *sendBuffer) {
	uint8_t count;
	uint16_t address;
	uint8_t index;
	...
	case APCI_MEMORY_WRITE_PDU:
		...
		if (apciCmd == APCI_MEMORY_WRITE_PDU) {
			if (processApciMemoryWritePDU(address, &telegram[10], count)) {
				...
				if ((getMaskVersion() > 0x1F)
						&& (userRam->deviceControl() & DEVCTRL_MEM_AUTO_RESPONSE)) {
	...
	case APCI_DEVICEDESCRIPTOR_READ_PDU:
		return (processDeviceDescriptorReadTelegram(sendBuffer,
				telegram[7] & 0x3f));
...
bool BcuDefault::processDeviceDescriptorReadTelegram(uint8_t *sendBuffer,
		int id) {
	if (id != 0) {
...
bool BcuDefault::processApciMasterResetPDU(uint8_t *sendBuffer,
		uint8_t eraseCode, uint8_t channelNumber) {
	...
	if (checkApciForMagicWord(eraseCode, channelNumber)) {
		...
		errorCode =
				(eraseCode == T_MASTERRESET_CONFIRMED_RESTART) ?
						T_RESTART_NO_ERROR : T_RESTART_UNSUPPORTED_ERASE_CODE;
