...
{
public:
	UserEepromMASK0701(Bus* bus) : UserEepromBCU2(bus, 0x3f00, 3072) {};
	virtual ~UserEepromMASK0701() = default;

    virtual int userEepromFlashSize() const;

protected:
	UserEepromMASK0701(Bus* bus, int start, int size) : UserEepromBCU2(bus, start, size) {};
	...
	union __attribute__ ((aligned (4)))
	{
		...
		962	byte commsSeg0Mcb[8];
		970	byte eibObjVer[5];      //!< Application programm 1 version
		975	byte commsSeg0Ver[5];   //!< Application programm 2 version
		};
...
inline int UserEepromMASK0701::userEepromFlashSize() const
{
	return 4096;
}
