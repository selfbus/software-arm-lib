...
public class CliOptions {
    ...
    private IndividualAddress progDevice = new IndividualAddress("15.15.192");
    ...
    public CliOptions(final String[] args, String helpApplicationName, String helpHeader, String helpFooter) throws ParseException, KNXFormatException {
        ...
        Option medium = Option.builder(OPT_SHORT_MEDIUM).longOpt(OPT_LONG_MEDIUM)
                ...
                .desc("KNX medium [tp1|rf] (default tp1)").build(); ///\todo not all implemented [tp0|tp1|p110|p132|rf]
        Option progDevice = Option.builder(OPT_SHORT_PROG_DEVICE).longOpt(OPT_LONG_PROG_DEVICE)
                .argName("progDevice")
                .hasArg()
                .required(false)
                .type(IndividualAddress.class)
                .desc(String.format("KNX device address in bootloader mode (default %s.%s.%s)",
                        Updater.PHYS_ADDRESS_BOOTLOADER_AREA,
                        Updater.PHYS_ADDRESS_BOOTLOADER_LINE,
                        Updater.PHYS_ADDRESS_BOOTLOADER_DEVICE)).build();
    ...
    private void parse(final String[] args) {
        ...
        try {
            ...
            if (cmdLine.hasOption(OPT_SHORT_NO_FLASH)) {
                NO_FLASH = true;
                logger.warn("--NO_FLASH => only erasePages and boot description block will be written");
            }
            logger.debug("NO_FLASH={}", NO_FLASH);

            if (cmdLine.hasOption(OPT_LONG_FULL)) {
                full = true;
            }
            ...
            // get the <KNX Interface>
            if (cmdLine.getArgs().length <= 0) {
                throw new ParseException("No <KNX Interface> specified.");
            } else {
            ...
            logger.error(this.helpToString(), e);
            System.exit(0);
        }
    }
